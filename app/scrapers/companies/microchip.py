...
class MicrochipScraper(BaseScraper):
        ...
        for term in self.SEARCH_TERMS:
            try:
                ...
                if first:
                    ...
                    debug_path = os.path.join(DEBUG_DIR, "microchip_rendered.html")
                    with open(debug_path, "w", encoding="utf-8") as f:
                        f.write(html)
                    print(f"[Microchip] Saved rendered HTML -> {debug_path}  ({len(html):,} bytes)")
                    first = False
                ...
                for card in cards:
                    ...
                    if not href.startswith("http"):
                        href = f"https://wd5.myworkdaysite.com{href}"
                ...
                print(f"[Microchip] term '{term}' failed: {e}")
